...
// pull in Mongoose model for rentals
...
// { rental: { title: '', text: 'foo' } } -> { rental: { text: 'foo' } }
...
router.get('/rentals', requireToken, (req, res, next) => {
  Rental.find()
    ...
    // respond with status 200 and JSON of the rentals
...
// SHOW
// GET /rentals/5a7db6c74d55bc51bdf39793
router.get('/rentals/:id', requireToken, (req, res, next) => {
  // req.params.id will be set based on the `:id` in the route
  Rental.findById(req.params.id)
    .then(handle404)
    // if `findById` is succesful, respond with 200 and "rental" JSON
...
router.post('/rentals', requireToken, (req, res, next) => {
  // set owner of new rental to be current user
  req.body.rental.owner = req.user.id

  Rental.create(req.body.rental)
    // respond to succesful `create` with status 201 and JSON of new "rental"
    .then(rental => {
...
router.patch('/rentals/:id', requireToken, removeBlanks, (req, res, next) => {
  ...
  Rental.findById(req.params.id)
    ...
    .then(rental => {
      ...
      return rental.update(req.body.rental)
    })
...
// DESTROY
// DELETE /rentals/5a7db6c74d55bc51bdf39793
router.delete('/rentals/:id', requireToken, (req, res, next) => {
  Rental.findById(req.params.id)
    .then(handle404)
    .then(rental => {
      // throw an error if current user doesn't own `rental`
      requireOwnership(req, rental)
      // delete the rental ONLY IF the above didn't throw
      rental.remove()
    })
